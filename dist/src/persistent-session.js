...
export class PersistentClaudeSession extends EventEmitter {
    ...
    async start() {
        ...
        if (this.options.debugFile)
            args.push('--debug-file', this.options.debugFile);
        if (this.options.fromPr)
            args.push('--from-pr', this.options.fromPr);
        if (this.options.channels) {
            const ch = Array.isArray(this.options.channels) ? this.options.channels : [this.options.channels];
            for (const c of ch)
                args.push('--channels', c);
        }
        if (this.options.dangerouslyLoadDevelopmentChannels) {
            const ch = Array.isArray(this.options.dangerouslyLoadDevelopmentChannels)
                ...
                : [this.options.dangerouslyLoadDevelopmentChannels];
            for (const c of ch)
                args.push('--dangerously-load-development-channels', c);
        }
                ...
                if (event.subtype === 'init') {
                    ...
                    this.stats.lastRetryError =
                        event.error_category ||
                            String(event.error_status || 'unknown');
                }
